...
function StudentOutlets() {
  ...
  try {
    ...
    const outlet = outlets.find(o => o.outlet_id === outletId);
    setSelectedOutlet(outlet);
    setOutletId(outletId); // ✅ important
  } catch (err) {
    console.error(err);
    alert("Failed to load menu");
  }
        ...
        <div className="menu-grid">
          {menuItems.map(item => (
            <div key={item.item_id} className="menu-item-card">
              ...
              <p>₹{item.price} {item.availability ? "(Available)" : "(Unavailable)"}</p>
    ...
    <div className="student-outlets-container">
      ...
      {outlets.map(outlet => (
        <div key={outlet.outlet_id} className="outlet-card">
          ...
          <p>Hours: {outlet.opening_time} - {outlet.closing_time}</p>
        ...
        {orders.length === 0 ? <p>No previous orders</p> :
          orders.map(order => (
            <div key={order.order_id} className="order-card">
              <p>Order ID: {order.order_id} | Status: {order.status} | Total: ₹{order.total_amount}</p>
              <ul>
                {order.items.map(i => (
                  <li key={i.order_item_id}>{i.item_name} x {i.quantity} = ₹{i.price * i.quantity}</li>
                ))}
              </ul>
            </div>
          ))
